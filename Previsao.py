...
def carregar_dados_padrao() -> pd.DataFrame:
    ...
    dados_vendas = {
        "Data": pd.date_range(start="2026-01-01", periods=12, freq="ME"), # 'ME' é o padrão atualizado do pandas para Month End
...
def treinar_modelo_tensorflow(X: np.ndarray, y: np.ndarray, epocas: int) -> tf.keras.Model:
    ...
    modelo = Sequential([
        Input(shape=(X.shape[1],)), # Prática recomendada em versões recentes do TF
